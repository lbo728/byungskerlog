      ...
      {posts.map((post: Post) => (
        <div key={post.id} className={`relative ${post.type !== "SHORT" ? "h-full" : ""}`}>
          <Link href={`/posts/${post.slug}`} className={`block group ${post.type !== "SHORT" ? "h-full" : ""}`}>
            <Card className={`flex flex-col overflow-hidden border-border/40 bg-card/50 hover:bg-card hover:shadow-md transition-all duration-300 group-hover:border-primary/50 py-0 pb-6 ${post.type !== "SHORT" ? "h-full" : ""}`}>
              {post.type !== "SHORT" && (
